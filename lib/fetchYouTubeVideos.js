...
const fetchYouTubeVideos = async(url) => {
  ...
  try{
    ...
    const res = await fetch(`${BASE_URL}/${url}&key=${process.env.YOUTUBE_API_KEY}`, {
...
export const getVideosByQuery = (sectionQuery = 'Section', limit = 10) => {
    const url = `search?part=snippet&maxResults=${limit}&q=${sectionQuery}&type=video`;
