...
import { fetchFavoriteVideosByUser, fetchWatchedVideosByUser } from './db/hasura'
...
export const fetchYouTubeVideos = async(url) => {
  ...
  return data?.items.map((movie) => {
      ...
      return {
          ...
          imgUrl: `https://i.ytimg.com/vi/${id.videoId || movie.id}/maxresdefault.jpg`,
          title: snippet.title,
...
export const getFavoriteVids = async(token, issuer) => {
  try{
    const favoriteVideos = await fetchFavoriteVideosByUser(token, issuer);
    
    if(favoriteVideos){
        const mapFavoriteVids = favoriteVideos.map((movie) => {
            return {
                id: movie.videoId,
                imgUrl: `https://i.ytimg.com/vi/${movie.videoId}/maxresdefault.jpg`,
            }
        })
        return mapFavoriteVids;
    } 
  }catch(err){
    console.error('something went wrong fetching favorite vids', err);
  }
}
